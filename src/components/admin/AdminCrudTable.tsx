    ...
    if (col.type === "select" && col.options) {
      return (
        <select value={String(value || "")} onChange={(e) => onChange(e.target.value)}
          className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm focus:ring-2 focus:ring-primary/30">
    ...
    if (col.type === "textarea") {
      return (
        <textarea value={String(value || "")} onChange={(e) => onChange(e.target.value)}
          className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm focus:ring-2 focus:ring-primary/30 min-h-[60px]" />
      );
    }
    return (
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={startCreate} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:shadow-lg hover:shadow-primary/25 transition-all">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..."
            className="w-full pl-9 pr-3 py-2 rounded-xl border border-border bg-card text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary/50" />
        </div>
        <span className="text-xs text-muted-foreground">{filteredRows.length} items</span>
      </div>

      {/* Create row */}
      {creating && (
        <div className="mb-4 p-4 rounded-xl border-2 border-primary/30 bg-primary/[0.03]">
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button onClick={() => setCreating(false)} className="px-4 py-2 rounded-lg border border-border text-sm hover:bg-muted/30">
              Cancel
            </button>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/30 border-b border-border">
                  {columns.map((col) => (
                    <th key={col.key} className="px-4 py-3 text-left font-medium text-muted-foreground text-xs uppercase tracking-wider" style={{ width: col.width }}>
                      {col.label}
                    </th>
                  ))}
                  <th className="px-4 py-3 text-right font-medium text-muted-foreground text-xs uppercase tracking-wider w-24">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredRows.map((row) => (
                  <tr key={row.id as string} className="border-b border-border last:border-0 hover:bg-muted/10 transition-colors">
                    {columns.map((col) => (
                      <td key={col.key} className="px-4 py-3">
                        {editingId === row.id ? (
                          ...
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${row[col.key] ? "bg-secondary/10 text-secondary" : "bg-muted text-muted-foreground"}`}>
                            {row[col.key] ? "Active" : "Inactive"}
                          </span>
                        ) : (
                          <span className="text-foreground line-clamp-2">{String(row[col.key] ?? "")}</span>
                        )}
                      </td>
                    ))}
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {editingId === row.id ? (
                          ...
                          <>
                            <button onClick={() => startEdit(row)} className="w-7 h-7 rounded-lg hover:bg-muted/50 text-muted-foreground hover:text-foreground flex items-center justify-center transition-colors">
                              <Pencil size={12} />
                            </button>
                            <button onClick={() => {
                ...
                {filteredRows.length === 0 && (
                  <tr><td colSpan={columns.length + 1} className="px-4 py-12 text-center text-muted-foreground">No items found</td></tr>
                )}
              </tbody>
            </table>
          </div>
