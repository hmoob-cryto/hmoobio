      ...
      <div className="container relative">
        <div className="text-center mb-14">
          ...
          <h2 className="font-display text-3xl sm:text-5xl font-bold mt-2">
            See How <span className="text-gradient-gold">Hmoob.io</span> Works
          </h2>
          <p className="text-muted-foreground text-lg mt-4 max-w-2xl mx-auto">
            Learn about our vision and cloud mining system that makes earning crypto simple for everyone
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto relative group rounded-2xl overflow-hidden border border-border hover:border-primary/20 transition-colors duration-500 shadow-2xl shadow-background/50">
          ...
          <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${playing ? "opacity-0 group-hover:opacity-100" : "opacity-100"}`}>
            <button
              onClick={togglePlay}
              className="w-20 h-20 rounded-full bg-primary/90 flex items-center justify-center hover:bg-primary hover:scale-105 transition-all duration-300 shadow-2xl shadow-primary/30 ring-4 ring-primary/20"
            >
              {playing ? (
                <Pause className="text-primary-foreground" size={30} />
              ) : (
                <Play className="text-primary-foreground ml-1" size={30} />
              )}
            </button>
          </div>
          ...
          <div className={`absolute bottom-5 right-5 transition-all duration-500 ${playing ? "opacity-0 group-hover:opacity-100" : "opacity-100"}`}>
            <button
              onClick={toggleMute}
              className="w-10 h-10 rounded-xl glass border border-border flex items-center justify-center text-foreground hover:text-primary hover:border-primary/30 transition-all duration-300"
            >
